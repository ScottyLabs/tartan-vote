...
3. **Proxy Setup** → **NEW** User declares senator status + optional proxy target via `ProxySetup.svelte`
...
2. **Session creation unchanged** — `POST /session/create` returns same response
