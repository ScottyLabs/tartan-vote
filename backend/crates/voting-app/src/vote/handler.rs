...
pub async fn create(
    ...
    let open_status = parse_status_option(&state.open_status)?;

    let model = event::ActiveModel {
        id: ActiveValue::NotSet,
        event_type: ActiveValue::Set(event_type),
        name: ActiveValue::Set(payload.name),
        status: ActiveValue::Set(open_status),
...
pub async fn close(
    ...
    let next_status = parse_status_option(&next_status)?;
...
pub async fn display_timer(
    ...
    let expired_from_time = end_time
        ...
        .map(|value| value <= &now)
        .unwrap_or(false);
    let expired_from_status = parse_status_option(&state.closed_status)
        .map(|closed| record.status == closed)
        .unwrap_or(false);

    Ok(Json(TimerResponse {
        record_id: record.id,
        status: status_option_to_string(record.status),
        now: now.to_rfc3339(),
...
fn parse_event_type(raw: &str) -> Result<EventType, ApiError> {
    match raw.trim().to_ascii_lowercase().as_str() {
        ...
        _ => Err(ApiError::BadRequest(
            "event_type must be one of: motion, election".to_owned(),
        )),
    }
}
...
fn parse_status_option(raw: &str) -> Result<StatusOption, ApiError> {
    match raw.trim().to_ascii_lowercase().as_str() {
        ...
        _ => Err(ApiError::BadRequest(
            "status must be one of: active, inactive".to_owned(),
        )),
    }
}
