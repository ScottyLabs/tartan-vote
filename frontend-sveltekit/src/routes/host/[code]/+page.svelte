<script lang="ts">
  ...
  let activeEvent = $state<null | { id: number; name: string; event_type: string }>(null);
  ...
  let m_meetingDisplay = $state<'named_by_category' | 'totals_only'>('named_by_category');
  ...
  // Export
  let exportKind = $state<'attendance-pdf' | 'attendance-csv' | 'votes-pdf' | 'votes-csv'>(
    'attendance-pdf'
  );
    ...
    try {
      ...
      const event = await api.createEvent(code, {
        ...
        data: {
          ...
          visibility: { participants: m_resultVisibility === 'live' ? 'live' : 'hidden_until_release' }
        }
      });
  ...
  async function doExport() {
    exporting = true;
    try {
      const [kind, format] = exportKind.split('-') as ['attendance' | 'votes', 'pdf' | 'csv'];
...
<HostShell sessionCode={code} active="overview" participantCount={headcount} onEndMeeting={() => (showEndConfirm = true)}>
  <div class="flex items-start justify-between gap-4 mb-6">
    <div>
      ...
      <h1 class="serif text-[40px] leading-[1.05] tracking-tight">Meeting overview</h1>
      <div class="text-[13px] text-ink-500 mt-1">
        Meeting code <b class="text-ink-900">{code}</b> · {headcount} in attendance
      </div>
    </div>
  ...
  <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
    <StatCard label="Participants" value={headcount} hint={`${headcount} checked in`} />
    <StatCard
      label="Active event"
    ...
    <div class="col-span-12 lg:col-span-7 card p-5">
      ...
      {#if attendees.length === 0}
        <div class="text-sm text-ink-500 py-6 text-center">
          No participants yet. Share the meeting code <b class="text-ink-900">{code}</b>.
        </div>
      {:else}
        <div class="divide-y divide-ink-200">
          {#each attendees as person}
            <div class="flex items-center gap-3 py-2.5">
              <div class="w-8 h-8 rounded-full bg-scarlet-100 text-scarlet-700 grid place-items-center text-xs font-bold">
                {(person.name ?? '??').toString().split(' ').map((w: string) => w[0]).join('').slice(0, 2).toUpperCase()}
              </div>
              <div class="flex-1">
              ...
              <span class="tag-pill">Weight {person.vote_instance_count ?? 1}</span>
    ...
    <div class="col-span-12 lg:col-span-5 space-y-5">
      <div class="card p-5">
        ...
        <div class="grid grid-cols-2 gap-3">
          <button class="btn-primary py-3.5 flex flex-col items-start px-4" onclick={() => (showMotion = true)}>
            <span class="flex items-center gap-1.5 text-[11px] uppercase tracking-widest opacity-80 font-semibold">
          ...
          <button class="btn-primary py-3.5 flex flex-col items-start px-4" onclick={() => (showElection = true)}>
            <span class="flex items-center gap-1.5 text-[11px] uppercase tracking-widest opacity-80 font-semibold">
              <svg width="12" height="12" fill="none" stroke="currentColor" stroke-width="2.2" viewBox="0 0 24 24">
                <path d="M8 7V5a4 4 0 118 0v2" /><rect x="4" y="7" width="16" height="14" rx="2" />
              </svg>
              Election
            </span>
      ...
      {#if activeEvent}
        <div class="card p-5">
          ...
          <Button full class="mt-4" onclick={() => goto(`/host/${code}/motion/${activeEvent!.id}`)}>
            Open live view
          </Button>
        </div>
      {/if}
...
>
  <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
    <div class="space-y-5">
      <div>
        ...
        <input id="m-title" class="input w-full" bind:value={m_title} placeholder="e.g. Approve the Q2 budget" />
      </div>
      <div class="grid grid-cols-2 gap-3">
        <SelectMenu
          label="Preset"
      ...
      <LongTextInput label="Description" bind:value={m_description} placeholder="Motion text as it will appear to voters." />
      <ArrayEditor label="Vote options" bind:items={m_voteOptions} placeholder="Option label" />
    </div>
    ...
    <div class="space-y-5">
      ...
      <label class="flex items-center gap-3 p-3 rounded-xl border border-ink-200 cursor-pointer hover:bg-ink-50">
        <input type="checkbox" class="hidden" bind:checked={m_proxy} />
        <span class="w-10 h-6 rounded-full relative transition" style="background: {m_proxy ? 'var(--scarlet-500)' : '#B7BECD'}">
          <span class="absolute top-0.5 w-5 h-5 rounded-full bg-white shadow transition-all" style="left: {m_proxy ? '18px' : '2px'}"></span>
        </span>
        <div>
          <div class="text-sm font-semibold">Enable proxy voting</div>
          <div class="text-[11px] text-ink-500">Delegates can cast on behalf of absent senators.</div>
        </div>
      </label>
  ...
  {#snippet footer()}
    <Button variant="ghost" size="sm" onclick={() => (showMotion = false)}>Cancel</Button>
    <div class="ml-auto">
...
>
  <div class="space-y-5">
    <div>
      ...
      <input id="e-title" class="input w-full" bind:value={e_title} placeholder="e.g. VP Finance 2026" />
    </div>

    <ArrayEditor label="Candidates" bind:items={e_candidates} addLabel="Add candidate" placeholder="Candidate name" />
  ...
  {#snippet footer()}
    <Button variant="ghost" size="sm" onclick={() => (showElection = false)}>Cancel</Button>
    <div class="ml-auto">
...
<!-- End meeting confirmation -->
<Modal open={showEndConfirm} title="End this meeting?" subtitle="All voters will be returned to the join screen and the session will close." onClose={() => (showEndConfirm = false)}>
  <div class="text-sm text-ink-500">
    Meeting code <b class="text-ink-900">{code}</b> · {headcount} attendee(s). You can still export records after closing.
  </div>
  {#snippet footer()}
    <Button variant="ghost" size="sm" onclick={() => (showEndConfirm = false)}>Cancel</Button>
    <div class="ml-auto">
